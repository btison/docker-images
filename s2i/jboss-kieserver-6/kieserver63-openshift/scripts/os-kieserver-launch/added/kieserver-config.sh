...
function getKieClassPath() {
    kieJarDir="${JBOSS_HOME}/standalone/deployments/kie-server.war/WEB-INF/lib"
    kieClassPath="."
        ...
        if [ -e ${modJarDir}/.overlays/.overlays ] ; then
            while read overlay ; do
                if [ -d ${modJarDir}/.overlays/${overlay}/${modJarPath}/main ] ; then
                    for modJar in `ls ${modJarDir}/.overlays/${overlay}/${modJarPath}/main/*.jar` ; do
...
function setKieContainerEnv() {
    ...
    eval "KIE_CONTAINER_REDIRECT_ENABLED=${kieContainerRedirectEnabled}"
    ...
    if [ "x${KIE_CONTAINER_DEPLOYMENT_OVERRIDE}" != "x" ]; then
        ...
        if [ "x${KIE_CONTAINER_DEPLOYMENT}" != "x" ]; then
            eval "KIE_CONTAINER_DEPLOYMENT_ORIGINAL=${KIE_CONTAINER_DEPLOYMENT}"
        ...
        eval "KIE_CONTAINER_DEPLOYMENT=${KIE_CONTAINER_DEPLOYMENT_OVERRIDE}"
        ...
        eval "KIE_CONTAINER_DEPLOYMENT=${kieContainerDeployments}"
    ...
    if [ "x${KIE_CONTAINER_DEPLOYMENT}" != "x" ]; then
        ...
        eval "KIE_CONTAINER_DEPLOYMENT_COUNT=${kieContainerDeploymentCount}"
        ...
        eval "KIE_CONTAINER_DEPLOYMENT_COUNT=0"
