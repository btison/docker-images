...
function check_view_pods_permission() {
    if [ -n "${OPENSHIFT_KUBE_PING_NAMESPACE+_}" ]; then
        ...
        pods_code=$(curl -s -o /dev/null -w "%{http_code}" -G --data-urlencode "${pods_labels}" -k -H "${pods_auth}" ${pods_url})
        if [ "${pods_code}" = "200" ]; then
            ...
            >&2 echo "WARNING: Service account has insufficient permissions to view pods in kubernetes (HTTP ${pods_code}). Clustering will be unavailable. Please refer to the documentation for configuration."
        else
            >&2 echo "WARNING: Service account unable to test permissions to view pods in kubernetes (HTTP ${pods_code}). Clustering will be unavailable. Please refer to the documentation for configuration."
        fi
    else
