...
export MYSQL_DEFAULTS_FILE=${MYSQL_DEFAULTS_FILE:-$HOME/my.cnf}
export MYSQL_BINLOG_FORMAT=${MYSQL_BINLOG_FORMAT:-STATEMENT}
export MYSQL_TRANSACTION_ISOLATION_LEVEL=${MYSQL_TRANSACTION_ISOLATION_LEVEL:=READ-COMMITTED}
...
function wait_for_mysql() {
  ...
  while [ true ]; do
    if [ -d "/proc/$pid" ]; then
      mysqladmin --socket=/tmp/mysql.sock ping &>/dev/null && echo "MySQL started successfully" && return 0
    else
      return 1
    fi
