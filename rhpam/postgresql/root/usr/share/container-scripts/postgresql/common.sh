...
function create_database() {
  if [[ -n ${POSTGRESQL_DATABASE} ]]; then
    for database in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${POSTGRESQL_DATABASE}"); do
      echo "Creating database: ${database}..."
      createdb --owner="$POSTGRESQL_USER" "$database"
      load_extensions ${database}
      execute_scripts ${database}
    done
  fi
}
...
function load_extensions() {
  ...
  if [[ -n ${POSTGRESQL_EXTENSION} ]]; then
    for extension in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${POSTGRESQL_EXTENSION}"); do
